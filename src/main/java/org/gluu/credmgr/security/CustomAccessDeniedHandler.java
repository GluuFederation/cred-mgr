...
/**
 * An implementation of AccessDeniedHandler by wrapping the
 * AccessDeniedHandlerImpl.
 *
 * In addition to sending a 403 (SC_FORBIDDEN) HTTP error code, it will remove
 * the invalid CSRF cookie from the browser side when a CsrfException occurs. In
 * this way the browser side application, e.g. JavaScript code, can distinguish
 * the CsrfException from other AccessDeniedExceptions and perform more specific
 * operations. For instance, send a GET HTTP method to obtain a new CSRF token.
 *
...
public class CustomAccessDeniedHandler implements AccessDeniedHandler {
	...
	if (accessDeniedException instanceof CsrfException && !response.isCommitted()) {
	    // Remove the session cookie so that client knows it's time to
